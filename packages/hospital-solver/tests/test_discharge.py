...
def test_documentation_demoted_at_peak_promoted_off_peak() -> None:
    ...
    doc_peak = next(i.priority for i in peak if i.stable_id == "discharge:doc1")
    doc_off = next(i.priority for i in off if i.stable_id == "discharge:doc1")
    ...
    max_discharge_rank = max(
        i.priority for i in off if i.priority is not None and i.stable_id != "discharge:doc1"
    )
